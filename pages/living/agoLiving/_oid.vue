<template>
  <div style="contentLiving">
    ...
    <div class="fl col-2 rightBox">
      <div v-if="flagIndex==1" id="msg" class="chat">
        <div v-for="(item, index) in messageList" :key="index" class="oneMsg">
          ...
          >
            ...
            <div class="textTypeOwn">
              <div class="other_user">{{ item.message }}</div>
            </div>
          </div>
          <div v-if="item.type == 2" class="textLine textFriend">
            ...
            <div class="textType">
              <div class="self-user">{{ item.message }}</div>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  asyncData({ params, error }) {
    return living.getLivingByIdAgo(params.oid).then(response => {
      return {
  ...
  mounted() {
    ...
    this.player = new Aliplayer(
      {
        id: "J_prismPlayer",
        vid: this.livingInfo.videoId, // 视频id
